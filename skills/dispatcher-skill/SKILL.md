...
5. After init creates the default `dispatcher_app/agents.json`, complete any known missing local agent fields before relying on the runtime. Keep the registry limited to `version`, `key_type`, and agent rows with `role_key`, `name`, `key`, and `key_status`; leave unknown handles as `null` with `key_status: "missing"` so the dispatcher can populate them later.
