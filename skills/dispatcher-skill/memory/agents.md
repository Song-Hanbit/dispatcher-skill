...
- Manager can invoke worker roles only by queueing dispatcher-owned worker requests with `dispatcher_app.worker_client`, which uses the dispatcher server API by default and keeps direct DB access as explicit `--transport db` fallback only.
...
- The dispatcher loop runs pending worker requests through `dispatcher_app/worker_runner.py` and stores dispatcher-owned worker Codex session handles in the ignored local `dispatcher_app/agents.json` as soon as the worker Codex session starts.
