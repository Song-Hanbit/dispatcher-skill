...
- `scripts/run_dispatcher_tunnel.py update-skill --repo <installed> --source <source> --candidate <candidate>` provides a dry-run-first package update helper that compares package files, preserves ignored runtime state, checks VERSION/CHANGELOG, runs package smoke by default, and applies only with `--confirm`.
...
- First-install operator flow allows required memory/requirements reads plus non-runtime smoke/init-status before runtime lock acquisition, then uses `python3 -m dispatcher_app.runtime_lock --db data/dispatcher.db ...` from the skill root before init writes local state. The runtime lock helper no longer creates `dispatcher_app/agents.json` as an import side effect, lock DB/token access failures include a concise writable-state hint, and token-file write failure rolls back the freshly acquired lock.
- `init-status` now includes a secret-free `next_init` block for uninitialized installs, using detected `cloudflared` when available and showing current, npx-install, source-repo, and skill-root init command templates plus password, port, and session guidance.
- `scripts/run_dispatcher_tunnel.py status --repo <skill-root>` is the post-init health summary: it reports local server HTTP status, expected tmux window presence, Quick Tunnel URL presence only, and safe restart candidates without printing the tunnel URL value.
- `init` now creates or updates a value-free `Dispatcher Skill Operator Baseline` block in the surrounding repository's `AGENTS.md`, preserving unrelated user instructions and handling both `.agents/skills/dispatcher-skill` and `skills/dispatcher-skill` installs.
