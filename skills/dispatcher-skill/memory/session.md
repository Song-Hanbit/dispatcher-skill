...
- Tmux session defaults use the surrounding repository name, not the `dispatcher-skill` directory name, so separate installed copies do not collide on one session.
